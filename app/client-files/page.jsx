...
export default function ClientFilesPage() {
  ...
  const [selectedCase, setSelectedCase] = useState("");   // ✅ added
  const [filterCase, setFilterCase] = useState("");       // ✅ added
      ...
      .then((data) =>
        setFiles(
          data.map((f) => ({
            ...
            url: f.url,
            caseId: f.caseId || null, // ✅ make sure caseId exists
          }))
        )
      );

    // ✅ fetch cases as well (replace with your API route if different)
  ...
  return (
    <main className="max-w-3xl mx-auto py-12 px-4">
      <h1 className="text-3xl font-bold mb-6 text-[#B68C5A]">
        Client File Management & Case Progress
      </h1>

      {/* Upload Section */}
      <section className="mb-8 p-4 border rounded-lg bg-[#f9f6f2]">
        <h2 className="text-xl font-semibold mb-2 text-[#B68C5A]">
          Upload Files
        </h2>
        <div className="mb-2">
        ...
        {uploading && (
          <div className="mt-2 w-full bg-gray-200 rounded h-2">
            <div
              className="bg-[#B68C5A] h-2 rounded"
      ...
      <section className="mb-8 p-4 border rounded-lg bg-[#f9f6f2]">
        ...
        <div className="flex flex-col md:flex-row md:space-x-4 mb-4">
          ...
          <select
            ...
            className="w-full md:w-60 border rounded px-2 py-2"
          >
            ...
            >
              <div className="flex items-center space-x-2">
                <a
                  ...
                  className="text-[#B68C5A] underline"
                >
                  {file.name}
                </a>
              ...
              <button
                ...
                onClick={async () => {
                  if (window.confirm("Delete this file?")) {
                    await fetch(
                      `/api/client-files/delete?id=${file.id}`,
                      { method: "DELETE" }
                    );
                    // Refresh file list
                    fetch("/api/client-files/list")
